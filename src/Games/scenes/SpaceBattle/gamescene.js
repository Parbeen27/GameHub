...
export default class GameScene extends Phaser.Scene{
    ...
    preload(){
        ...
        this.load.pack("game","SpaceBattle/Assets.json")
    }
    ...
    create(){
        ...
        //==================================controlls==========================================
        this.isTouchDevice = this.sys.game.device.input.touch || /Mobi|Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
    ...
    resize(gameSize){
        const {width: w, height: h} = gameSize || {width: this.scale.width, height: this.scale.height};
        // Player
        this.player.setPosition(w/2, h-100);
        
        // Joystick repositioning
        if (this.joyBase && this.joyStick) {
            this.joyBaseX = w * 0.15;
            this.joyBaseY = h * 0.8;
            this.joyBase.setPosition(this.joyBaseX, this.joyBaseY);
            this.joyStick.setPosition(this.joyBaseX, this.joyBaseY);
        }
        
        // Shoot button
        if (this.shootButton) {
            this.shootButton.setPosition(w * 0.85, h * 0.85);
        }
    ...
    mobileconstrols(){   
        ...
        // Joystick setup
        this.joyBaseRadius = Math.max(40, W * 0.05);
        this.joyKnobRadius = Math.max(20, W * 0.025);
        this.joyBaseX = W * 0.15;
        this.joyBaseY = H * 0.8;
        this.joyMaxDist = this.joyBaseRadius;
        this.joyPointer = null;

        // Joystick base
        this.joyBase = this.add.circle(this.joyBaseX, this.joyBaseY, this.joyBaseRadius, 0x888888, 0.7).setScrollFactor(0).setInteractive().setStrokeStyle(2, 0xffffff, 0.5);
        this.joyStick = this.add.circle(this.joyBaseX, this.joyBaseY, this.joyKnobRadius, 0xcccccc, 1).setScrollFactor(0).setStrokeStyle(2, 0x000000, 0.8);

        // Joystick input
        this.input.on('pointerdown', (pointer) => {
            if (pointer.x < W / 2 && !this.shootButton.getBounds().contains(pointer.x, pointer.y)) {
    ...
    mobilemovement(){
        ...
        if (this.joyPointer) {
            ...
            const dist = Math.sqrt(dx * dx + dy * dy);
            const normalizedDist = dist / this.joyMaxDist;

            if (normalizedDist > 0.1) { // Dead zone
                this.player.setVelocityX((dx / dist) * speed * normalizedDist);
                this.player.setVelocityY((dy / dist) * speed * normalizedDist);
                targetAngle = Math.atan2(dy, dx) * (180 / Math.PI) * 0.1; // Slight tilt
                ismoving = true;
            } else {
                this.player.setVelocity(0, 0);
            }
