...
export default class GameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.pack("game1","PlatformGame/Assets.json");
    ...
    create() {
     ...
     //   this.player.body.setSize(this.player.width * 0.6,this.player.height * 0.8);
        ...
        this.ammo.getChildren().forEach((child)=> {
            let x = Phaser.Math.Between(50, 3000 - 50);
            child.setPosition(x, Phaser.Math.Between(0, 100));
            child.setBounceY(Phaser.Math.FloatBetween(0.2, 0.5));
            child.setScale(.05);
     ...
     //   this.setupCamera();
        ...
        if(this.sys.game.device.os.android || this.sys.game.device.os.iOS){
            ...
            this.rotate = this.add.text(width/2, height/2, 'Rotate and reload device for best experience in this game', {   fontSize: '22px', fill: '#f51818', align: 'center',fontStyle: 'bold' }).setOrigin(0.5).setScrollFactor(0).setDepth(1001).setWordWrapWidth(width * 0.8);
            this.time.delayedCall(2500, () => {
                this.rotate.setVisible(false); // Show if in portrait
            });
        }
        //back button
        ...
        this.backButton = this.add.text(this.scale.width - 100, 20, "||",{
            fontSize: '24px',
            backgroundColor: '#000',
            padding: { x: 8, y: 4 },
            color: '#fff'
    ...
    resize(gameSize){
        ...
        // this.createPlatforms(h); // Removed: don't recreate platforms on resize
        ...
        if(this.floor){
            ...
            const floorYPosition = h + 140; // Place below viewport
            this.floor
        ...
        this.boss.setPosition(3000, h - 100); // Keep boss above floor
        
        // Don't reposition player/enemies on resize to avoid disrupting gameplay
        
        // this.physics.add.collider(this.player,this.platforms); // Already added in create
        // this.physics.add.collider(this.enemy,this.platforms);
        // this.physics.add.collider(this.boss, this.platforms);  
        
        // UI Text repositioning (set scroll factor to 0 so they stay on screen)
        const uiX = w * 0.02; // 2% from left edge
        const uiYStart = 50;
        if(this.scoreText){
            this.scoreText.setPosition(uiX, uiYStart).setScrollFactor(0);
        }
        if(this.playerHealthtext){
            this.playerHealthtext.setPosition(uiX, uiYStart + 50).setScrollFactor(0);
        }
        if(this.ammoText){
            this.ammoText.setPosition(uiX, uiYStart + 100).setScrollFactor(0);
        }
        if(this.powertext){
            this.powertext.setPosition(uiX, uiYStart + 150).setScrollFactor(0);
        }
        
        // Back button repositioning
        if(this.backButton){
            this.backButton.setPosition(w - 100, h * 0.02).setScrollFactor(0);
        }
        ...
        setupCamera() {
            ...
            this.cameras.main.setZoom(1); // Fixed zoom to prevent display changes on rotation
